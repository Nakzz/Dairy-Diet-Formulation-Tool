...
'''
from dietcal_march_13_data import *
...
LOCAL = True
if LOCAL != True:
    print "Vector before order: "
    print "CONS_RHS_MAX_2_php: "
    print CONS_RHS_MAX_2_php
    print "CONS_LHS_MIN_2_php"
    print CONS_LHS_MIN_2_php

#Reorder Horizontal constrainst, DM constraints is first
...
def objfunc(x):
    ...
    for j in range(2,len(NUTRIENTS_php)+1):
        g_index = (j-2)*2+2
...
for i in range(1,len(CONS_LHS_MIN_1_php)+1):
        x_value = 'x' + str(i)
        opt_prob.addVar(x_value,'c',lower=float(CONS_LHS_MIN_1_php[i-1]),upper=float(CONS_RHS_MAX_1_php[i-1]),value=1)
opt_prob.addObj('f')
