...
//            console.dir();
	...
	formattedPrices['Ingredient'] = 'Solution: '; //Solution Word
		...
		if(ingredientID < 7 && ingredientID > 0) {
		    console.log(solutionVector[ingredientID-1]);
		    dm_forage = dm_forage + parseFloat(cur_dm*solutionVector[ingredientID-1]);console.log(dm_forage);
		    ndf_forage = ndf_forage + parseFloat(NDF*cur_dm*solutionVector[ingredientID-1]);
	...
	ndf_forage = ndf_forage + ' %DM NDF from forage';
    ...
    loadComplete: function () {
		...
		feedvalGrid.grid.jqGrid('setCell', i, 'Min_kgcowd', tmp);
                ...
                $('#grid tr[id="'+i+'"]').removeClass('ui-state-highlight');
		$('#grid tr[id="'+i+'"]').addClass('ui-state-highlight-forage');
		//alert(i);	
            }
            rawData = feedvalGrid.grid.jqGrid('getRowData');
	    console.log(rawData);
